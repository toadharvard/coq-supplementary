...
Proof.
  intros z EVAL.
  destruct (defined_expression e s z id EVAL ID) as [z' DEF].
  specialize (UNDEF z').
  contradiction.
Qed.
...
induction e; intros.
  ...
  - inversion E1. inversion E2. subst. eapply state_deterministic in VAR. all: eauto.
...
Lemma variable_relevance (e : expr) (s1 s2 : state Z) (z : Z)
  ...
  [| e |] s2 => z.
  Proof. 
    induction EV; econstructor.
    specialize (FV i). apply FV. apply v_Var.
    all: eauto.
Qed.
...
Proof. intro. symmetry. eauto. Qed.

Lemma eq_trans (e1 e2 e3 : expr) (EQ1 : e1 ~~ e2) (EQ2 : e2 ~~ e3):
  e1 ~~ e3.
Proof. 
  split; intro H.
  - apply EQ2, EQ1. assumption.
  - apply EQ1, EQ2. assumption.
Qed.
